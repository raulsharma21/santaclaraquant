...
class MultiAssetPortfolioOptimization(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.longAssets = {
            ...
            #"FIN":  self.AddEquity("UYG", Resolution.Daily).Symbol,      # Financials
            "ENERGY": self.AddEquity("ERX", Resolution.Daily).Symbol,    # Energy
            "IND": self.AddEquity("DNI", Resolution.Daily).Symbol,       # Industrials
            "HEALTH": self.AddEquity("UHC", Resolution.Daily).Symbol       # Healthcare
        }
        
        # 4) Rotation ETFs: SSO and TBF.
        self.ssoSymbol = self.AddEquity("SSO", Resolution.Daily).Symbol
        ...
        # 7) Indicators for rotation and individual signals:
        # SSO 200-day SMA for rotation logic.
        self.sso200SMA = self.SMA(self.ssoSymbol, 200, Resolution.Daily)
        # 20-day MA for each leveraged ETF (for long signals).
        self.momentumMA = {}
        for symbol in self.longAssets.values():
            self.momentumMA[symbol] = self.SMA(symbol, 20, Resolution.Daily)
        # 20-day volume SMA for leveraged ETFs (retained for compatibility).
        self.volumeSMA = {}
    ...
    def OnData(self, data):
        ...
        if forwardEY is not None and tenYearYield is not None:
            ...
            if forwardEY < tenYearYield:
                assetsToClear = list(self.longAssets.values()) + [self.ssoSymbol, self.tbfSymbol] + list(self.bondCandidates.values())
        ...
        # --- Next, evaluate individual leveraged ETF signals (only long signals now).
        bullishSymbols = []
        for symbol in self.longAssets.values():
        ...
        # If there are individual bullish signals, exit any rotation positions.
        if bullishSymbols:
            self.Liquidate(self.ssoSymbol)
            ...
            # If no clear individual long signals exist, revert to the rotation logic.
    ...
    def RotateRotation(self, data):
        """
        When no individual signals are present:
        - If SSO is above its 200-day SMA, hold SSO exclusively.
        - Otherwise, hold the bond rotation: select the current maximum-yield bond
          (using real fundamental data) and immunize it with TBF by dynamically rebalancing
          to achieve a net duration of 0.
        """
        # Liquidate any individual asset positions.
        for name, symbol in self.longAssets.items():
            if self.Portfolio[symbol].Invested:
                self.Liquidate(symbol)
                self.CancelOrdersForSymbol(symbol)
        
        if not self.sso200SMA.IsReady:
            return
        if self.ssoSymbol not in data or data[self.ssoSymbol] is None:
            return
        
        ssoPrice = data[self.ssoSymbol].Price
        sso200   = self.sso200SMA.Current.Value
        
        if ssoPrice > sso200:
            # Use SSO exclusively.
            for bond in self.bondCandidates.values():
                if self.Portfolio[bond].Invested:
                    self.Liquidate(bond)
            if self.Portfolio[self.tbfSymbol].Invested:
                self.Liquidate(self.tbfSymbol)
            if not self.Portfolio[self.ssoSymbol].Invested:
                self.SetHoldings(self.ssoSymbol, 1.0)
                self.Debug("SSO above 200-day SMA => rotating into SSO.")
        else:
            # Use bond rotation.
            ...
            if self.Portfolio[self.ssoSymbol].Invested:
                self.Liquidate(self.ssoSymbol)
            for bond in self.bondCandidates.values():
